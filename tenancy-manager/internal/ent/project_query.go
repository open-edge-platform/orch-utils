...
func (_q *ProjectQuery) QueryFolder() *FolderQuery {
...
func (_q *ProjectQuery) WithFolder(opts ...func(*FolderQuery)) *ProjectQuery {
	query := (&FolderClient{config: _q.config}).Query()
...
func (_q *ProjectQuery) loadFolder(ctx context.Context, query *FolderQuery, nodes []*Project, init func(*Project), assign func(*Project, *Folder)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Project)
	for i := range nodes {
		if nodes[i].folder_projects == nil {
			continue
		}
		fk := *nodes[i].folder_projects
