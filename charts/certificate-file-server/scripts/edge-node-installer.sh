...
# Source configuration (will be provided by ConfigMap)
if [ -f "/tmp/installer-config.env" ]; then
    source /tmp/installer-config.env
elif [ -f "./installer-config.env" ]; then
    source ./installer-config.env
else
...
if [ -n "${EN_PROXY_HTTP:-}" ]; then
    grep -qF "http_proxy" /etc/environment || echo "http_proxy=${EN_PROXY_HTTP}" >> /etc/environment
    grep -qF "https_proxy" /etc/environment || echo "https_proxy=${EN_PROXY_HTTPS}" >> /etc/environment
fi
if [ -n "${EN_PROXY_FTP:-}" ]; then
    grep -qF "ftp_proxy" /etc/environment || echo "ftp_proxy=${EN_PROXY_FTP}" >> /etc/environment
fi
if [ -n "${EN_PROXY_SOCKS:-}" ]; then
    grep -qF "socks_proxy" /etc/environment || echo "socks_proxy=${EN_PROXY_SOCKS}" >> /etc/environment
fi
if [ -n "${EN_PROXY_NO_PROXY:-}" ]; then
    grep -qF "no_proxy" /etc/environment || echo "no_proxy=${EN_PROXY_NO_PROXY}" >> /etc/environment
fi
...
NODE_ACCESS_URL=${ORCH_KEYCLOAK}
NODE_RS_URL=${ORCH_RELEASE}
NODE_SERVICE_CLIENTS=${EN_SERVICE_CLIENTS}
NODE_OUTBOUND_CLIENTS=${EN_OUTBOUND_CLIENTS}
NODE_METRICS_ENABLED=${EN_METRICS_ENABLED}
NODE_TOKEN_CLIENTS=${EN_TOKEN_CLIENTS}
CADDY_APT_PROXY_URL=$(echo "$FILE_SERVER" | cut -d: -f1)
CADDY_APT_PROXY_PORT=$(echo "$FILE_SERVER" | cut -d: -f2)
REGISTRY_URL=$(echo "$REGISTRY" | cut -d: -f1)
CADDY_REGISTRY_PROXY_URL=$(echo "$REGISTRY" | cut -d: -f1)
CADDY_REGISTRY_PROXY_PORT=$(echo "$REGISTRY" | cut -d: -f2)
PLATFORM_MANAGEABILITY_URL=${ORCH_DEVICE_MANAGER}
PLATFORM_MANAGEABILITY_METRICS=${EN_METRICS_ENABLED}
RPS_ADDRESS=$(echo "$RPS_HOST" | cut -d: -f1)
KEYCLOAK_FQDN=$(echo "$KEYCLOAK" | cut -d: -f1)
RELEASE_FQDN=$(echo "$RELEASE" | cut -d: -f1)
RS_TYPE=${RS_TYPE}
DEB_PACKAGES_REPO=${EN_DEBIAN_PACKAGES_REPO}
FILE_RS_ROOT=${EN_FILES_RS_ROOT}
EOF
...
install_device_discovery(){
    if grep -q "install_device_discovery done" "$SCRIPT_DIR"/$STATUS_FILENAME; then
        ...
        echo "device-discovery-agent device-discovery-agent/auth.keycloakURL string ${ORCH_KEYCLOAK}" | debconf-set-selections
        ...
        oras pull "${REGISTRY_SERVICE}/${EN_DEBIAN_PACKAGES_REPO}/device-discovery-agent:${DEVICE_DISCOVERY_VERSION}"
...
get_rs_token() {
    if [ "$RS_TYPE" == "auth" ]; then
        ...
        # Get RS token now
        tempfile=$(mktemp)
        curl -XGET https://${ORCH_RELEASE}/token -H "Authorization: Bearer $AT" -o "$tempfile"
...
install_node_agent(){
    if grep -q "install_node_agent done" "$SCRIPT_DIR"/$STATUS_FILENAME; then
        ...
        echo "node-agent node-agent/status.serviceClients string ${EN_SERVICE_CLIENTS}" | debconf-set-selections
        echo "node-agent node-agent/status.outboundClients string ${EN_OUTBOUND_CLIENTS}" | debconf-set-selections
        echo "node-agent node-agent/metrics.enabled string ${EN_METRICS_ENABLED}" | debconf-set-selections
        echo "node-agent node-agent/auth.accessTokenURL string ${ORCH_KEYCLOAK}" | debconf-set-selections
        echo "node-agent node-agent/auth.rsTokenURL string ${ORCH_RELEASE}" | debconf-set-selections
        echo "node-agent node-agent/auth.tokenClients string ${EN_TOKEN_CLIENTS}" | debconf-set-selections
        ...
        echo "node-agent node-agent/proxy.aptSourceFilesRSRoot string ${EN_FILES_RS_ROOT}" | debconf-set-selections
        ...
        if [ "$RS_TYPE" == "auth" ]; then
            echo "${RS_AT}" | oras pull "${REGISTRY_SERVICE}/${EN_DEBIAN_PACKAGES_REPO}/caddy:${CADDY_VERSION}" --password-stdin
        else
            oras pull "${REGISTRY_SERVICE}/${EN_DEBIAN_PACKAGES_REPO}/caddy:${CADDY_VERSION}"
        fi
        ...
        echo "download node agent"
         if [ "$RS_TYPE" == "auth" ]; then
            echo "${RS_AT}" | oras pull "${REGISTRY_SERVICE}/${EN_DEBIAN_PACKAGES_REPO}/node-agent:${NODE_AGENT_VERSION}" --password-stdin
        else
            oras pull "${REGISTRY_SERVICE}/${EN_DEBIAN_PACKAGES_REPO}/node-agent:${NODE_AGENT_VERSION}"
        fi
        ...
        while true; do
            http_status=$(curl -s -o /dev/null -w "%{http_code}" "http://localhost:${ORCH_APT_SRC_PROXY_PORT}/${EN_FILES_RS_ROOT}/edge-node.asc")
        ...
        if [ "$RS_TYPE" == "auth" ]; then
            curl -kfsSL "https://$CADDY_APT_PROXY_URL/${EN_FILES_RS_ROOT}/edge-node.asc" -H "Authorization: Bearer $RS_AT" -o /etc/apt/trusted.gpg.d/edge-node.asc
        else
            curl -kfsSL "https://$CADDY_APT_PROXY_URL/${EN_FILES_RS_ROOT}/edge-node.asc" -o /etc/apt/trusted.gpg.d/edge-node.asc
        fi
        echo "deb http://localhost:${ORCH_APT_SRC_PROXY_PORT}/${EN_FILES_RS_ROOT}/repository $APT_DISTRO main" | tee /etc/apt/sources.list.d/edge-node.list
...
install_PMA() {
    if grep -q "install_PMA done" "$SCRIPT_DIR"/$STATUS_FILENAME; then
        ...
        if echo "$amt_output" | grep -iqE "AMT Pro Corporate|Intel Standard Manageability Corporate" > /dev/null; then
            echo "platform-manageability-agent platform-manageability-agent/manageability.serviceURL string ${ORCH_DEVICE_MANAGER}" | debconf-set-selections
            echo "platform-manageability-agent platform-manageability-agent/rpsAddress string string $RPS_ADDRESS" | debconf-set-selections
            echo "platform-manageability-agent platform-manageability-agent/metrics.enabled string ${EN_METRICS_ENABLED}" | debconf-set-selections
