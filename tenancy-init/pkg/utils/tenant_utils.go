...
func DeleteOrg(ctx context.Context, org string) error {
	orgId, _ := getOrgId(ctx, org)
	if orgId == "" {
		log.Info().Msgf("Org (%s) not present to delete. Skipping delete \n", org)
		return nil
	}
	config := ctrl.GetConfigOrDie()
	nexusClient, err := nexus_client.NewForConfig(config)
	...
	for {
		...
		case <-ticker.C:
			...
			if nexus_client.IsNotFound(err) {
				log.Info().Msg("org not found, deleted successfully")
				return nil
			}
		case <-timeout:
...
func DeleteProject(ctx context.Context, org, project string) error {
	projectId, _ := GetProjectId(ctx, org, project)
	if projectId == "" {
		log.Info().Msgf("Project (%s) not present to delete. Skipping delete \n", project)
		return nil
	}
	...
	for {
		...
		case <-timeout:
			log.Info().Msg("project deletion timed out")
